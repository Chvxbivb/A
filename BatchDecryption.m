...
for k = 1:length(imageFiles)
    ...
    load (fullfile(newFolderName, 'ProcessedKey.mat'));
    img = imread(fullfile(newFolderName, 'encrypted_img.tiff'));
    %% 密文图像
    tic;
    EncryptedImg = DecryptColorImg(img,xR_0, xG_0, xB_0, xR_1, xG_1, xB_1, xP_0, xP_1);
    toc;
% 创建加密图像的文件名（EPS 格式）
    newFileName = fullfile(newFolderName, 'decrypted_img.tiff');
